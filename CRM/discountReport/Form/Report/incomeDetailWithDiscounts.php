...
class CRM_discountReport_Form_Report_incomeDetailWithDiscounts extends CRM_Report_Form_Event {
  ...
  public function buildEventReport($eventIDs) {
    ...
    try {
      ...
      CRM_Core_Session::setStatus(ts('The CiviDiscount API does not exist, discount code functionality in this report will be turned off'), ts('CiviDiscount is not enabled'), "error");
	  $discountExtensionOff = 1;
    }
	
    $rows = $eventSummary = $roleRows = $statusRows = $instrumentRows = $discountRows = $count = array();
	...
	if($discountExtensionOff == 0) {
      //Count Participants by CiviDiscount code for Event
      $discountUse = "SELECT cividiscount_track.entity_id, 
                        cividiscount_track.item_id, 
                        civicrm_participant.event_id as event_id,
                        SUM(civicrm_participant.fee_amount) as amount,
                        cividiscount_item.id as discount_code, 
                        COUNT(cividiscount_item.id) as discount_total
                    FROM cividiscount_track 
                    LEFT JOIN civicrm_participant ON (cividiscount_track.entity_id = civicrm_participant.id)
                    LEFT JOIN cividiscount_item ON (cividiscount_item.id = cividiscount_track.item_id)
                    WHERE civicrm_participant.event_id IN ( {$eventID} )
					AND civicrm_participant.is_test  = 0
                    {$activeParticipantClause}
                    GROUP BY cividiscount_item.code, civicrm_participant.event_id
                    ";
                    
      $discountDAO = $this->executeReportQuery($discountUse);
    
      while ($discountDAO->fetch()) {
        $discountRows[$discountDAO->event_id][$discountCodes[$discountDAO->discount_code]]['total'] = $discountDAO->discount_total;
        $discountRows[$discountDAO->event_id][$discountCodes[$discountDAO->discount_code]]['round'] = round(($discountDAO->discount_total / $count[$discountDAO->event_id]) * 100, 2);
        $discountRows[$discountDAO->event_id][$discountCodes[$discountDAO->discount_code]]['amount'] = CRM_Utils_Money::format($discountDAO->amount, $currency[$discountDAO->event_id]);
      }
      $rows['Discount Codes'] = $discountRows;
    }
